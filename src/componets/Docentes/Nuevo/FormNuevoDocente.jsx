...
export const FormNuevoDocente = () => {
  ...
  const Schema = Yup.object().shape({
    ...
    prefixCUIL: Yup.number("Ingrese un número").required("Campo requerido").test('len', 'Debe tener 2 dígitos', val =>
      val ? val.toString().length === 2 : false
    ),
    dni: Yup.number("Ingrese un número").required("Campo requerido").test('len', 'Debe tener 8 dígitos', val =>
      val ? val.toString().length === 8 : false
    ),
    postfixCUIL: Yup.number("Ingrese un número").required("Campo requerido").test('len', 'Debe tener 1 dígito', val =>
  ...
  const newDocente = (values,resetForm)=>{
    ...
    axios.post("http://127.0.0.1:8000/api/v1/docentes",values,{
      headers: {
  ...
  return (
    ...
    <form className="max-w-md mx-auto" onSubmit={handleSubmit}>
     <h1 className=" text-3xl font-mono font-bold text-blue-900 mb-8">Nuevo Docente</h1>
      ...
      <div className="relative z-0 w-full mb-5 group mt-4">
        ...
        {errors.finicioDocencia && (
              <p className=" text-red-600 text-xs">{errors.finicioDocencia}</p>
          )}
      </div>
